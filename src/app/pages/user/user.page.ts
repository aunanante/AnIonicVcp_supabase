...
export class UserPage implements OnInit {
  ...
  constructor(
    ...
    this.businessOwnerForm = this.formBuilder.group({
      ...
      telephone2: [''],
      monthly_fee_paid: [{ value: true, disabled: true }],
    });
  ...
  async updateBusinessOwner() {
    ...
    if (currentUserEmail !== null && currentUserEmail !== undefined) {
      ...
      const modal = await this.modalController.create({
        ...
        componentProps: { businessOwnerData: this.businessOwnerForm.value },
      });

      modal.onDidDismiss().then((result) => {
        if (result.data === true) {
          // User confirmed, proceed with the update
          this.performBusinessOwnerUpdate(currentUserEmail);
        }
      });
  ...
  async loadBusinessOwners() {
    if (this.userEmail) {
      this.businessOwners = await this.userService.getBusinessOwnersByCurrentUser(
        this.userEmail
      );
